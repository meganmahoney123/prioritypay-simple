...
export async function sendDepositAlertPush({ token, depositAmount, dashboardUrl }) {
  ...
  return sendPush(token, {
    ...
    body: `You just received a ${amountStr} deposit! Open the app to distribute your split.`,
    url: dashboardUrl,
  });
}

// Distinct from sendDepositAlertPush above -- see lib/sms.js's
...
export async function sendTransferLandedPush({ token, amount, destLabel, dashboardUrl }) {
  const amountStr = Number(amount).toLocaleString("en-US", { style: "currency", currency: "USD" });
  return sendPush(token, {
    title: "Transfer landed",
    body: `Your ${amountStr} transfer to ${destLabel} has landed.`,
    url: dashboardUrl,
  });
}
