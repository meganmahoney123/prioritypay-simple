...
export const ledgerInputStyle = (extra = {}) => ({
  ...
  // any input rendering under 16px, which is jarring on every text/
  // number field across the authenticated app and the public quiz's
  // email step. Callers can still override via `extra` where a field
  // is genuinely display-only.
  fontSize: 16,
  color: "var(--color-text)",
