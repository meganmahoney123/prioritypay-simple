...
const RULES = {
  ...
  "solo-401k-contribution-deadlines": {
    test: (a) => a.retirement === "self_employed_no_plan_yet" || a.retirement === "self_employed_has_solo401k_or_sep",
    condition: () => "You're managing your own Solo 401(k) as a self-employed person.",
    action: () => "You could confirm your plan's election and funding deadlines before year-end, rather than assuming they match a W-2 401(k)'s timeline.",
    benefit: () => "Missing a self-employed plan's specific deadlines can mean losing the ability to make a contribution for the year at all.",
    scenario: () => "Someone who opens a Solo 401(k) in November and documents their contribution election by December 31 can still fund it up until their extended tax filing deadline the following year, rather than needing the cash in hand before year-end.",
  },
  "cash-balance-plan": {
  ...
  "qbi-section-199a-deduction": {
    ...
    condition: () => "You own a pass-through business and your income is near or above roughly $190,000 single or $380,000 married.",
    action: () => "You could confirm how much of the 20% QBI deduction you actually qualify for, especially if you're in a specialty service field.",
    benefit: () => "This is an automatic deduction on top of the standard deduction, but it phases out based on income and how much the business pays in W-2 wages.",
    scenario: () => "A sole proprietor with $150,000 of qualified business income below the phase-out threshold could deduct 20%, or $30,000, directly from taxable income, on top of the standard deduction.",
  },
  "ptet-election": {
  ...
  "fbar-fatca-international-reporting": {
    ...
    condition: () => "You hold foreign bank or investment accounts, or live abroad.",
    action: () => "You could confirm whether you're required to file an FBAR or FATCA report based on your account balances.",
    benefit: () => "These are separate from your regular tax return, and getting ahead of them avoids penalties that can run into the thousands even for an honest mistake.",
    scenario: () => "Someone with $15,000 spread across two foreign bank accounts is required to file an FBAR, since the combined total exceeds the $10,000 threshold, even though neither account alone crosses that line.",
  },
  ...
  "equity-vesting-and-ownership-percentage": {
    test: (a) => a.equity?.includes("startup_equity_or_options"),
    condition: () => "You hold startup equity.",
    action: () => "You could confirm your exact vesting schedule and your real ownership percentage directly with the company.",
    benefit: () => "New funding rounds dilute your percentage even when the dollar value is rising, so this is the number that actually tells you what you own.",
    scenario: () => "An employee granted 50,000 shares out of 10,000,000 total shares outstanding owns 0.5%, but that percentage shrinks with every new funding round that issues additional shares, even if the per-share price is rising.",
  },
  ...
  "post-termination-exercise-period": {
    test: (a) => a.equity?.includes("startup_equity_or_options"),
    condition: () => "You're considering leaving a startup with unexercised vested options.",
    action: () => "You could confirm the plan's post-termination exercise window before you give notice.",
    benefit: () => "A short window can force you to pay the strike price and exercise on a tight deadline or forfeit the options entirely.",
    scenario: () => "Leaving a company with 20,000 vested options at a $2 strike price under a standard 90-day PTEP means finding $40,000 to exercise within 3 months or forfeiting the options entirely. A company offering a multi-year PTEP removes that time pressure.",
  },
  "qsbs-overview": {
  ...
  "qsbs-requirements": {
    test: (a) => a.equity?.includes("c_corp_founder_or_early_employee"),
    condition: () => "You hold C-corp stock and want to make sure it still qualifies for QSBS.",
    action: () => "You could check the company's asset test and business activity against QSBS requirements before any stock buyback or large financing round.",
    benefit: () => "Certain company actions can accidentally break QSBS eligibility, and confirming it early avoids finding out only after a sale.",
    scenario: () => "A company buying back $10,000,000 of stock from a departing cofounder without checking the QSBS asset test first could accidentally push total assets over the $75,000,000 ceiling, jeopardizing QSBS status for every other shareholder.",
  },
  "qsbs-rollover-1045": {
