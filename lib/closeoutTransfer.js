...
export async function fireCloseoutTransfer({ admin, userId, fromAccountId, toAccountId, amount, label, retirementType = null }) {
  ...
  const { data: transfer, error: transferError } = await admin
    .from("transfers")
    .insert({ user_id: userId, source_amount: amt, status: "processing", trigger: "closeout" })
    .select("id")
    .single();
  ...
  try {
    ...
    await admin.from("transfers").update({ status: "failed" }).eq("id", transfer.id);
    return { error: err?.body || err?.message || String(err), status: 500 };
  }

  await admin.from("transfer_allocations").insert({
    transfer_id: transfer.id,
    category_type: "closeout",
  ...
  await admin.from("transfers").update({ status }).eq("id", transfer.id);
