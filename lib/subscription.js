...
export function isBusinessPlan(profile) {
  ...
  // Entitled while actively subscribed OR still inside the trial window --
  // mirrors isReadOnly()'s trial-aware shape so a Business subscriber on a
  // Stripe trial (should the Business price ever add one) isn't locked out
  // of the features they're paying toward. Today Business checkout starts
  // active with no trial, so in practice this resolves on the active branch.
