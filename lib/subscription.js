...
// an action (connecting a new Plaid account, recording a real transfer)
// and by the UI (Settings billing section, dashboard banner).
//
...
// Business tier gate (PHASE T). Deliberately its own check, not folded
...
export function isBusinessPlan(profile) {
  ...
  // `plan` only becomes 'business' via a completed Business checkout, whose
  // webhook sets subscription_status to 'active' at the same time, so an
  // active subscription IS the entitlement. An earlier version also granted
  // access whenever trial_ends_at was still in the future -- but that column
  // is the *Simple*-signup 30-day trial and it persists through a
  // cancellation, so it would have kept a canceled Business subscriber
  // entitled until day 30. If the Business price ever adds its own trial,
  // gate on a Business-specific trial signal here, not the Simple one.
