...
// Guards the OTHER direction of the same invariant as checkAccountRoomForLabel
// above: pulling money OUT of an account's "Unallocated" cash (rather than
// crediting a category) must never leave that account's OTHER categories
// over its real balance. An account with a $5,000 real balance but $3,000
// already set aside for Retirement and $2,000 for Tax Reserve has $0 of
// ACTUALLY unallocated cash left, even though nothing previously stopped
// someone from wiring $500 of it out via a real ACH transfer (see POST
// /api/allocations/execute-real-transfer) -- checkAccountRoomForLabel alone
// never caught this because that credit-side check only runs against the
// DESTINATION category, and a transfer sourced from Unallocated has no
// destination-side category account to check when the two sides live in
// different accounts. When over room, also returns `otherLabels` -- the
// categories actually holding this account's money right now -- so the
// caller can name them in the "transfer from one of those instead" message.
export async function checkAccountUnallocatedRoom(admin, userId, accountId, amountToRemove) {
  const { data: account } = await admin
    .from("simple_accounts")
    .select("current_balance")
    .eq("id", accountId)
  ...
  const otherLabels = Object.entries(byLabel)
    .filter(([, v]) => v > 0.005)
    .map(([label]) => label);
  const categorized = Object.values(byLabel).reduce((s, v) => s + Math.max(0, v), 0);
  const room = Math.round((accountBalance - categorized) * 100) / 100;

  if (amountToRemove > room + 0.005) {
    return { ok: false, accountBalance, categorized: Math.round(categorized * 100) / 100, room: Math.max(0, room), otherLabels };
  }
  return { ok: true };
}
