...
// lib/ledgerTheme.js -- LEDGER_TOKENS is still used by accounts, split
// rules, settings, and every calculator page not yet redesigned, so
// those must keep reading the old values untouched.
...
export const bloomPrimaryButtonStyle = (extra = {}) => ({
  ...bloomButtonBase,
...
export const bloomGhostButtonStyle = (extra = {}) => ({
  ...bloomButtonBase,
...
export const bloomBadgeStyle = (extra = {}) => ({
  display: "inline-flex",
  alignItems: "center",
  gap: 6,
  fontFamily: "var(--font-heading)",
...
// Plum-toned accent callout (matches the dashboard's "Total saved" hero
// panel and other prominent, non-alarming highlighted boxes).
export const bloomAccentCardStyle = (extra = {}) => ({
...
// Lilac notice card -- e.g. "these categories need an account before money
// can move," or "N days left in your free trial." Less severe than
// bloomWarningCardStyle.
export const bloomNoticeCardStyle = (extra = {}) => ({
  border: "1px solid var(--color-accent-300)",
  borderRadius: "var(--radius-md)",
  background: "var(--color-accent-200)",
  color: "var(--color-accent-800)",
...
// Shared color pair for "money leaving this account" vs "money landing in
// this account" call-outs -- used by the real-transfer confirmation popup
// (app/(app)/transfers/page.js) and the persistent pending-transfer banner
// (components/AppShell.js) to make which side is which unambiguous at a
// glance. MOVE_OUT reuses the exact warm red bloomWarningCardStyle already
// established for "this needs your attention" callouts; MOVE_IN is a
// green kept just saturated enough to read as "landing here" without
// fighting the purple accent used everywhere else.
export const MOVE_OUT_COLOR = "#9C3B22";
export const MOVE_IN_COLOR = "#1F7A4D";

// Warm-red warning card -- reserved for the dashboard's non-dismissible
// "trial ended," "not contributing to retirement/investments" callouts,
// kept out of the purple palette on purpose so these still read as more
// urgent than a routine notice.
export const bloomWarningCardStyle = (extra = {}) => ({
