...
// of Solo 401k's one -- Traditional 401(k), Traditional IRA, and HSA are
// all core/locked for this persona the same way Solo 401k is core for
// everyone else, since these are the three accounts PriorityPay
...
// personas and harmless for the W2 ones too (their Traditional 401(k)/
// IRA/HSA rows just wouldn't be locked, same as any other custom row,
// rather than crashing or mis-locking Solo 401k on their behalf).
export function isCoreRow(rule, persona) {
  ...
  if (isW2NoSideHustle(persona)) {
    ...
    if (rule.label === "Traditional 401(k)") return rule.retirementType === "traditional_401k";
    if (rule.label === "Traditional IRA") return rule.retirementType === "traditional_ira";
...
// Persona-aware starting split rules -- everyone else keeps the original
// six-row DEFAULT_SPLIT_RULES (Solo 401k as the one default retirement
// sub-account, exactly as before), but W2 (No Side Hustle/Business) has
// no self-employment income to open a Solo 401k or SEP IRA against, so
// its Retirement group starts with the three accounts that ARE available
// to a plain W2 employee -- Traditional 401(k), Traditional IRA, and HSA
// -- splitting the same 15% Retirement allocation three ways (5% each)
// rather than leaving it all on an inapplicable Solo 401k. W2 (With Side
// Hustle/Business) keeps the original Solo 401k default (side income is
// still self-employment income), but can add Traditional 401(k)/IRA/HSA
// as additional sub-accounts the same way anyone can add a SEP IRA today
// -- see RETIREMENT_LABELS below, which now includes all five types.
export function getDefaultSplitRules(persona) {
  if (!isW2NoSideHustle(persona)) return DEFAULT_SPLIT_RULES;
  return {
    fixed: [],
    percent: [
      { id: "tax_reserve", label: "Tax Reserve", group: null, pct: 20, max: null, balanceCap: null, color: "#a3a3a3", accountId: null, startingBalance: null },
      { id: "investments_1", label: "Investments", group: "Investments", pct: 10, max: null, balanceCap: null, color: "#14b8a6", accountId: null, startingBalance: null },
      { id: "traditional_401k", label: "Traditional 401(k)", group: "Retirement", pct: 5, max: null, balanceCap: null, color: "#8b5cf6", accountId: null, retirementType: "traditional_401k", startingBalance: null },
      { id: "traditional_ira", label: "Traditional IRA", group: "Retirement", pct: 5, max: null, balanceCap: null, color: "#6366f1", accountId: null, retirementType: "traditional_ira", startingBalance: null },
...
export const RETIREMENT_GROUP_SUBTEXT_W2_NO_SIDE_HUSTLE =
  "Traditional 401(k), Traditional IRA, and HSA are the retirement (and HSA) accounts available to a W2 employee. You can set up these accounts after you finish onboarding.";
