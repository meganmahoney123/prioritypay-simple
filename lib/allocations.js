// Shared allocation engine -- imported both server-side (the run-split
// route, so what actually gets recorded is provably the same math the
...
// Same purple ramp as the approved MainDesktop/AccountsADesktop mockups
// and the app's own accent tokens (lib/bloomTheme.js's --color-accent-300/
// 700/800 are the 1st/5th/6th entries here) -- one on-brand hue at
// increasing depth, instead of the old rainbow (green/blue/purple/pink/
// amber/red/etc) that made every pie chart and category list look like it
// was using a different, uncoordinated color per row.
...
// Deterministic, effectively-unlimited color generator so the Nth
// category/account color is always visually distinct from every color
// before it, no matter how many rows someone creates -- CATEGORY_COLORS is
// the first, hand-picked batch (the exact mockup ramp); generatedColor()
// takes over once that's exhausted so nothing after row 6 ever collides
// the way a plain `% CATEGORY_COLORS.length` wrap-around would. Stays
// within the same purple family (hue ~262, matching the palette above)
// with lightness/saturation cycling to stay distinguishable, rather than
// the old golden-angle full-spectrum rotation -- a category list longer
// than the curated palette should still read as "the app's colors," not
// an arbitrary rainbow.
function generatedColor(index) {
  const i = index - CATEGORY_COLORS.length;
  // Locked to the curated ramp's own hue (~258, confirmed by converting
  // each hex above to HSL -- all six land at 257.6-260.0) instead of the
  // old +7/24-step wobble (258-281), which drifted the hue toward pink
  // the higher the index climbed. That wobble is what made every one of
  // Megan's real categories (all of which hash past index 5 into this
  // fallback -- see colorForLabel) render as a visibly different, washed
  // "generated" purple instead of the app's actual purple, even though
  // the curated CATEGORY_COLORS ramp itself was always correct. Lightness
  // now cycles across the same ~18-88% span the curated ramp spans
  // (D9C9FF=89% down to 3B1C7A=29%), and saturation wobbles a little so
  // two overflow shades at similar lightness still read as distinct.
  const lightness = 88 - ((i * 11) % 70); // cycles ~18-88%
  const saturation = 65 + ((i * 7) % 30); // 65-95%
  return `hsl(258, ${saturation}%, ${lightness}%)`;
}
export function colorForIndex(index) {
  return index < CATEGORY_COLORS.length ? CATEGORY_COLORS[index] : generatedColor(index);
}

// Deterministic color purely from a category's label text, independent of
// its position in any particular query result. Two different screens
// (the Dashboard's pie, an individual account's mini-donut on the
// Accounts page) fetch a category's row from two different API routes
// that don't guarantee the same row order or even the same subset of
// categories -- an index-based color (colorForIndex(i)) could quietly
// assign "Tax Reserve" a different shade on each screen. Hashing the
// label itself instead means the same category always gets the same
// purple shade everywhere, with zero shared state between fetches. Also
// used in place of trusting simple_split_rules_percent.color directly --
// that column can hold a color assigned before the all-purple ("Bloom")
// redesign, which would otherwise still win and show up as a stray
// gray/teal/orange slice.
export function colorForLabel(label) {
  ...
  return colorForIndex(Math.abs(hash) % 30);
}
...
// "Retirement (Side Income)" (added Sept 2026) is a second grouped bucket,
// used only by W2 (With Side Hustle/Business) -- that persona has a
// workplace retirement lineup (401k/IRA/HSA, under plain "Retirement",
// same as W2 No Side Hustle) AND self-employment income from the side
// hustle, which can separately fund a Solo 401k -- see getDefaultSplitRules
// below. Every other persona only ever produces rows in plain "Retirement".
export const GROUPED_BUCKETS = ["Investments", "Retirement", "Retirement (Side Income)"];
...
// W2 (With Side Hustle/Business) gets BOTH retirement lineups (Sept 2026
// product decision): the workplace 401k/IRA/HSA every W2 employee has
// access to (same as W2 No Side Hustle, group "Retirement"), PLUS a Solo
// 401k for the side income itself, since side income is still
// self-employment income (group "Retirement (Side Income)" -- see
// GROUPED_BUCKETS above, which is what actually splits these into two
// separate labeled sections instead of one). Also drops "Business Expenses
// (OPEX)" from its core defaults, same reasoning as W2 (No Side Hustle/
// Business): a W2 employee's side hustle isn't assumed to carry its own
// dedicated operating-expense bucket the way a full business is.
  ...
  if (isW2WithSideHustle(persona)) {
    return {
      ...
      percent: [
        { id: "tax_reserve", label: "Tax Reserve", group: null, pct: 20, max: null, balanceCap: null, color: "#a3a3a3", accountId: null, startingBalance: null },
        { id: "investments_1", label: "Investments", group: "Investments", pct: 10, max: null, balanceCap: null, color: "#14b8a6", accountId: null, startingBalance: null },
        // Workplace lineup (group: "Retirement") -- same three accounts as
        // W2 No Side Hustle, since a side hustle doesn't change what's
        // available through the day job. Kept separate from the Solo 401k
        // below (group: "Retirement (Side Income)") so onboarding/Split
        // Rules render them as two distinctly-labeled sections instead of
        // one bucket that reads like it's all one thing.
        { id: "traditional_401k", label: "401k", group: "Retirement", pct: 5, max: null, balanceCap: null, color: "#8b5cf6", accountId: null, retirementType: "traditional_401k", startingBalance: null },
        { id: "traditional_ira", label: "IRA", group: "Retirement", pct: 5, max: null, balanceCap: null, color: "#6366f1", accountId: null, retirementType: "traditional_ira", startingBalance: null },
        { id: "hsa", label: "HSA", group: "Retirement", pct: 5, max: null, balanceCap: null, color: "#0ea5e9", accountId: null, retirementType: "hsa", startingBalance: null },
        { id: "solo_401k", label: "Solo 401k", group: "Retirement (Side Income)", pct: 10, max: null, balanceCap: null, color: "#ec4899", accountId: null, retirementType: "solo_401k", startingBalance: null },
        { id: "emergency_fund", label: "Emergency Fund", group: "Savings", pct: 10, max: null, balanceCap: null, color: "#f59e0b", accountId: null, startingBalance: null },
        { id: "savings", label: "Savings", group: "Savings", pct: 10, max: null, balanceCap: null, color: "#ef4444", accountId: null, startingBalance: null },
      ],
    };
...
export function newSubAccountRow(group, usedColors = []) {
  ...
  return {
    ...
    label: group.startsWith("Retirement") ? "New retirement account" : "New investment",
    group,
    pct: 0,
    max: null,
    balanceCap: null,
...
export const RETIREMENT_GROUP_SUBTEXT_W2_NO_SIDE_HUSTLE =
  "401k, IRA, and HSA are the retirement (and HSA) accounts available to a W2 employee. Check with your employer first -- many already route money to these automatically through payroll. If so, you don't need to send anything here for that; just connect the real accounts below so PriorityPay can show their balance growing over time, and double-check with HR/payroll so you don't end up over-contributing between payroll and what you set here.";

// W2 (With Side Hustle/Business)'s workplace lineup gets the same
// employer-check framing as W2 No Side Hustle above (both personas have a
// day job that may already be funding these) -- this group is separate
// from RETIREMENT_GROUP_SUBTEXT_W2_WITH_SIDE_HUSTLE_SIDE below (the Solo
// 401k section), which is about the side income instead.
export const RETIREMENT_GROUP_SUBTEXT_W2_WITH_SIDE_HUSTLE_JOB = RETIREMENT_GROUP_SUBTEXT_W2_NO_SIDE_HUSTLE;

// The Solo 401k section for W2 (With Side Hustle/Business) -- explicitly
// distinguishes this from the workplace lineup above, since someone could
// otherwise read "another retirement section" as duplicate/redundant.
export const RETIREMENT_GROUP_SUBTEXT_W2_WITH_SIDE_HUSTLE_SIDE =
  "A Solo 401k is the retirement account for your side income specifically. It's separate from your workplace 401k/IRA/HSA above, since side income is its own self-employment income the IRS treats differently. You can set this up after you finish onboarding.";

// `group` is the exact GROUPED_BUCKETS string this subtext is being shown
// under -- needed now that W2 (With Side Hustle/Business) has two
// differently-labeled retirement sections (see GROUPED_BUCKETS above) that
// each need their own copy, not just one persona-wide default.
export function retirementGroupSubtext(persona, group = "Retirement") {
  if (group === "Retirement (Side Income)") return RETIREMENT_GROUP_SUBTEXT_W2_WITH_SIDE_HUSTLE_SIDE;
  if (isW2NoSideHustle(persona) || isW2WithSideHustle(persona)) return RETIREMENT_GROUP_SUBTEXT_W2_NO_SIDE_HUSTLE;
  return RETIREMENT_GROUP_SUBTEXT;
}

// Rows connecting a REAL retirement/HSA account (not a savings account
// holding money for later manual transfer) -- only the W2 workplace lineup
// (401k/IRA/HSA funded by payroll) qualifies. Solo 401k/SEP IRA keep the
// original savings-holding pattern (see RetirementNote/connectSavingsOnly)
// since self-employment income has no payroll deduction to piggyback on.
// Used to scope the Plaid Link flow to real retirement/investment accounts
// instead of a plain savings account for these specific rows -- see
// PercentRow in components/PercentSplitEditor.js.
export function isEmployerRetirementRow(rule) {
  return rule?.retirementType === "traditional_401k" || rule?.retirementType === "traditional_ira" || rule?.retirementType === "hsa";
}
...
// Whether a category's money actually needs to move is decided by the
// caller (the run-split route): if the account chosen for it is the same
// account the deposit landed in, there's nothing to move -- it's already
// there.
...
// Shared by the Split Rules page (app/(app)/splits/page.js, to decide
// whether Save should be blocked) and PercentSplitEditor's own per-row
// warning card -- one implementation so the two surfaces can never drift
// apart on what counts as "over budget." A Starting Balance is a claim
// about real money already sitting in ONE connected account (see PUT
// /api/split-rules's own server-side version of this same check, which
// is what actually enforces it -- this is the client-side mirror that
// surfaces the same problem before someone hits Save); this sums every
// row's claim per account and compares it to that account's own real
// balance, exactly the way two categories both drawing from the same
// bank account would have to share what's actually in it.
export function computeStartingBalanceRoom(percent, accounts) {
  const accountsById = Object.fromEntries((accounts || []).map((a) => [a.id, a]));
  const byAccountId = {};
  (percent || []).forEach((r) => {
    if (!r.accountId) return;
    const sb = Number(r.startingBalance) || 0;
    if (!byAccountId[r.accountId]) byAccountId[r.accountId] = [];
    byAccountId[r.accountId].push({ id: r.id, label: r.label, startingBalance: sb });
  });
  const byRuleId = {};
  const overAccountIds = [];
  Object.entries(byAccountId).forEach(([accountId, rows]) => {
    const account = accountsById[accountId];
    const accountBalance = account ? Number(account.current_balance) || 0 : null;
    const total = rows.reduce((s, r) => s + r.startingBalance, 0);
    const isOver = accountBalance !== null && total > accountBalance + 0.005;
    if (isOver) overAccountIds.push(accountId);
    rows.forEach((r) => {
      const others = rows.filter((x) => x.id !== r.id);
      const othersSum = others.reduce((s, x) => s + x.startingBalance, 0);
      byRuleId[r.id] = {
        accountId,
        accountLabel: account ? `${account.institution_name} \u2022\u2022\u2022\u2022 ${account.mask}` : null,
        accountBalance,
        rows,
        total,
        isOver,
        room: accountBalance === null ? null : Math.max(0, accountBalance - othersSum),
        otherLabels: others.filter((x) => x.startingBalance > 0).map((x) => x.label),
      };
    });
  });
  return { byRuleId, anyOver: overAccountIds.length > 0, overAccountIds };
}
