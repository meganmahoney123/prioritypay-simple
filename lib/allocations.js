...
export const DEFAULT_SPLIT_RULES = {
  ...
  percent: [
    { id: "tax_reserve", label: "Tax Reserve", group: null, pct: 20, max: null, color: "#a3a3a3", accountId: null },
    { id: "investments_1", label: "Investments", group: "Investments", pct: 10, max: null, color: "#14b8a6", accountId: null },
    { id: "solo_401k", label: "Solo 401k", group: "Retirement", pct: 10, max: null, color: "#8b5cf6", accountId: null, retirementType: "solo_401k" },
    { id: "sep_ira", label: "SEP IRA", group: "Retirement", pct: 5, max: null, color: "#ec4899", accountId: null, retirementType: "sep_ira" },
    { id: "emergency_fund", label: "Emergency Fund", group: "Savings", pct: 10, max: null, color: "#f59e0b", accountId: null },
    { id: "opex", label: "Business Expenses (OPEX)", group: null, pct: 10, max: null, color: "#7c3aed", accountId: null },
    { id: "savings", label: "Savings", group: "Savings", pct: 10, max: null, color: "#ef4444", accountId: null },
  ],
};
...
// retirement account") can be renamed or removed. Distinguishing by id
// instead of some `core: true` flag keeps this a pure function of data
// already in `percent`, so it works the same in onboarding and Split Rules
// without either page having to track anything extra.
export const CORE_PERCENT_IDS = new Set([
  "tax_reserve",
  "investments_1",
  "solo_401k",
  "sep_ira",
...
export function isCoreRow(id) {
  return CORE_PERCENT_IDS.has(id);
}
...
export function clampPctToRemaining(percent, id, requestedPct) {
  ...
  const roomLeft = Math.max(0, 100 - others);
...
export function pctTotal(percentRules) {
  return percentRules.reduce((s, r) => s + (Number(r.pct) || 0), 0);
}
...
//
// A percent category's `max` is a MONTHLY TOTAL cap, not a per-deposit one
// -- every deposit that lands this month keeps chipping away at the same
// cap (e.g. three $500 paychecks against a $1,000 cap fund the category in
// full on the second deposit, and $0 on the third, with the extra
// redistributed proportionally to everything else, same as if a single big
// deposit had hit the cap). `percentAllocatedThisMonth` is an optional map
// of { [percentRowId]: amountAlreadyGivenThisMonth-before-this-deposit },
// supplied by the caller (lib/runSplit.js, which knows what's already been
// sent this month via transfer_allocations) -- client-side previews don't
// have that history so they pass nothing and only ever see a single
// deposit's worth of room under the cap, same simplification already used
// for the retirement monthly cap.
export function computeAllocations(splitRules, amount, percentAllocatedThisMonth = {}) {
  ...
  while (remaining > 0.005 && open.length > 0 && guard < percent.length + 1) {
    ...
    open.forEach((r) => {
      ...
      const room = Math.max(0, max - givenEarlierThisMonth - allocated[r.id]);
      if (share >= room) {
        allocated[r.id] += room;
        givenThisRound += room;
        anyCapped = true;
