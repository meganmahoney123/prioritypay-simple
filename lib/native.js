...
// Lightweight, side-effect-free permission check for UI use (see
// AppLockGate.js's "enable notifications" banner) -- doesn't request or
// register anything, just reports where things stand: "prompt" (never
// asked), "granted", or "denied". iOS only shows its native permission
// dialog once ever per install; once someone taps "Don't Allow" there's no
// API to make it reappear, so a denied status is what tells the UI to show
// its own reminder pointing at Settings instead of calling
// requestPermissions() again (which would just silently resolve "denied"
// with no dialog at all).
export async function getPushPermissionStatus() {
  if (!(await isNativeApp())) return "granted";
  try {
...
// Deep-links into this app's page in iOS Settings. There's no Capacitor
// plugin API for this (and no native plugin installed just for it) -- but
// `app-settings:` is a URL scheme WKWebView-based apps can still navigate
// to directly to get there, same as Safari/Mail's own "Notifications ->
// [App]" deep links.
export async function openNotificationSettings() {
  ...
  try {
    ...
    // no-op -- worst case someone has to open Settings manually.
  }
}
...
export async function registerForPushNotifications(onNotificationTap) {
  ...
  try {
    ...
    // iOS's registerForRemoteNotifications() doesn't guarantee its
    // did-register/did-fail callback fires on every call -- especially
    // right at cold launch, before the device's network/APNs courier
    // connection is fully up. Apple's own guidance is that apps should
    // just keep asking rather than treat one call as authoritative, so
    // this retries with backoff until a token (or a real error) comes
    // back, and tries again whenever the app returns to the foreground
    // if it still doesn't have one (covers: opened app before network was
    // ready, backgrounded it, reopened later).
    let settled = false;
    let attempt = 0;
    let retryTimer = null;
    const maxAttempts = 4;
    const retryDelaysMs = [3000, 6000, 12000, 20000];

    const tryRegister = () => {
      if (settled) return;
      attempt += 1;
      console.log(`[push] register attempt ${attempt}`);
      PushNotifications.register();
      clearTimeout(retryTimer);
      if (attempt < maxAttempts) {
        retryTimer = setTimeout(tryRegister, retryDelaysMs[attempt - 1]);
      }
    };
    ...
    PushNotifications.addListener("registration", async (token) => {
      settled = true;
      clearTimeout(retryTimer);
      console.log(`[push] registered after ${attempt} attempt(s)`);
      try {
    ...
    PushNotifications.addListener("registrationError", (err) => {
      // A real error (denied entitlement, no network, etc) -- log it, but
      // keep retrying on the same backoff schedule rather than giving up
      // after one failure, since these can be transient too.
      console.warn(`[push] registration attempt ${attempt} failed`, err);
    });
    ...
    try {
      ...
      // @capacitor/app not available -- foreground retry just doesn't happen;
      // the launch-time retries above still run.
    }
