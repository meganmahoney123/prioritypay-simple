...
export async function fetchTransactionsForMonth({ accessToken, realmId, year, month }) {
  ...
  const colTypes = (report?.Columns?.Column || []).map((c) => (c.ColType || "").toLowerCase());
  const idxOf = (...names) => {
    for (const n of names) {
      const i = colTypes.indexOf(n);
      if (i >= 0) return i;
    }
    return -1;
  };
  const iDate = idxOf("tx_date", "date");
  const iType = idxOf("txn_type", "transaction_type");
  const iName = idxOf("name");
  const iAmount = idxOf("subt_nat_amount", "subt_nat_home_amount", "amount");
