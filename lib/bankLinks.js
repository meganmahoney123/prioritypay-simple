...
// Best-effort match: lowercase the institution name, then look for the
// first known key that appears as a whole-word match (\b...\b), NOT just
// any substring. A plain .includes() check is too loose here -- Plaid's
// sandbox institution "First Platypus Bank" contains the literal characters
// "us bank" (the end of "platyp-us" + " bank"), which would wrongly match
// the real US Bank entry with a naive substring check. Word-boundary
// matching avoids that class of false positive while still matching
// "Ally" inside "Ally Bank", "Chase" inside "Chase Business Checking", etc.
// Falls back to a Google search for the bank's login page, which is at
// worst one extra click and never a dead link -- far better than silently
// rendering nothing for a bank not yet in the map above.
export function resolveBankLoginUrl(institutionName) {
  ...
  for (const key of Object.keys(BANK_LOGIN_URLS)) {
    ...
    if (pattern.test(name)) return BANK_LOGIN_URLS[key];
  }
