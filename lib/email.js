...
// Distinct from sendDepositAlertEmail above -- see lib/sms.js's
// sendTransferLandedSms for why this needs clearly different copy: this
// confirms a transfer the user already approved has arrived, not that new
// money showed up needing a split.
export async function sendTransferLandedEmail({ toEmail, amount, destLabel, dashboardUrl }) {
  const amountStr = Number(amount).toLocaleString("en-US", { style: "currency", currency: "USD" });
  return sendEmail({
    to: toEmail,
    subject: `Your ${amountStr} transfer has landed`,
    text: `Your ${amountStr} transfer to ${destLabel} has landed. View it at ${dashboardUrl}.`,
    html: `<p>Your <strong>${amountStr}</strong> transfer to <strong>${destLabel}</strong> has landed.</p><p><a href="${dashboardUrl}">View it in PriorityPay →</a></p>`,
  });
}
