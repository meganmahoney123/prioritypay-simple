  ...
  if (hasPending) {
    try {
      const { data: notifyProfile } = await admin
        .from("simple_profiles")
        .select("phone_number, sms_notifications_enabled, sms_threshold, email_notifications_enabled")
        .eq("id", userId)
        .single();
      ...
      // SMS_ALERTS_ENABLED above). Sent to the account's login email
      // (auth.users, fetched via the admin API since simple_profiles
      // doesn't duplicate it) rather than a separately-collected address,
      // so there's nothing new for someone to type in.
