...
export async function runSplit({
  ...
  const [{ data: percentRows }, { data: accounts }] = await Promise.all([
    admin.from("split_rules_percent").select("*").eq("user_id", userId),
    admin.from("accounts").select("id, dwolla_funding_source_id, current_balance").eq("user_id", userId),
  ]);
  ...
  const { data: monthPercentAllocations } = await admin
    ...
    .select("amount, label, transfers!inner(user_id, created_at)")
    .eq("transfers.user_id", userId)
    .gte("transfers.created_at", monthStartIso)
    .neq("status", "failed")
  ...
  const { allocated } = computeAllocations(splitRules, total, percentAllocatedThisMonth);
  ...
  async function processCategory(rule, categoryType) {
    ...
    const canMove = destAccount?.dwolla_funding_source_id && destAccount.id !== sourceAccountId;
  ...
  if (allocationRows.length) {
    await admin.from("transfer_allocations").insert(allocationRows);
  }

  const finalStatus = failures.length ? "failed" : "processing";
  await admin.from("transfers").update({ status: finalStatus }).eq("id", transfer.id);
