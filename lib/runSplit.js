...
// PHASE W: re-enabled. Text alerts were on hold while business
// verification for text-capable numbers was stuck (first Twilio A2P
// 10DLC, then Telnyx toll-free verification -- see lib/sms.js's own
// history note). The toll-free verification rejection ("Business
// Registration Number Is Missing or Invalid") is being resent to Telnyx
// separately; this flag is independent of that and just controls whether
// this codebase attempts to send at all. sendDepositAlertSms still
// no-ops safely if TELNYX_API_KEY/TELNYX_FROM_NUMBER aren't set (see
// lib/sms.js), so flipping this back on is safe even before the number is
// verified -- nothing actually sends until Telnyx approves the number and
// the env vars are set.
const SMS_ALERTS_ENABLED = true;
...
// the constant now lives there so the Plaid link-token routes can check it
// too, without pulling in everything this file imports.
...
// record the deposit, and record every category whose account differs
// from where the deposit landed as a transfer the user needs to go make
// themselves.
//
...
export async function runSplit({
  ...
  const [{ data: percentRows }, { data: accounts }] = await Promise.all([
    ...
    admin.from("simple_accounts").select("id, current_balance, institution_name, account_name, mask").eq("user_id", userId),
  ]);
  ...
  async function processCategory(rule, categoryType) {
    ...
    // that isn't where it already landed. PriorityPay never originates
    // that move itself in any case -- it just calculates it and records
    // it, so a connected account works fine as a checklist destination
    // with nothing further required of it.
    ...
    if (needsMove) {
      // This row just records what the split calculated so the
      // "Transfers waiting on you" checklist (see PendingTransfers
      // component) can tell the user what to send and where, and so this
      // becomes real, provable transaction history the user confirms
      // themselves once they've actually sent it.
    ...
    allocationRows.push({
      ...
      label: rule.label,
      amount: amt,
      // Snapshot of the split-rule-computed amount, kept even if `amount`
      // is later overridden by the user (see
      // app/api/transfer-allocations/[id]/amount/route.js) -- see
      ...
      // Snapshotted here, once, so PendingTransfers.js's "Transfers
      // waiting on you" checklist can still say where this was headed
      // even after simple_accounts.id goes null (dest_account_id is "on
      // delete set null" -- PHASE G) because the account was later
      // disconnected or replaced. See PHASE U, supabase/schema.sql.
      dest_account_label: destAccountId ? `${destAccount.institution_name} ${destAccount.account_name} •••• ${destAccount.mask}` : null,
      // Same snapshot pattern as dest_account_label above, for the
      // account this deposit actually landed in -- the bank someone
      // needs to log into to send this category's slice of it onward.
      // See supabase/migrations/20260923_source_account_label.sql.
  ...
  // Deposit-threshold alerts -- opt-in, set in Settings > Notifications.
  // Fires on every auto-detected deposit (trigger === "auto_deposit"),
  // regardless of whether anything is left needing a manual transfer
  // (hasPending) -- everything here is manual (no auto-execution), so
  // "needs_approval" only reflects whether a category has a destination
  // account connected yet, not whether the user should be told money
  // landed. Deliberately skipped for trigger === "manual" (the "Split $X
  // now" button) -- notifying someone about a split they just personally
  // clicked to run is redundant, same reasoning as the threshold skip
  // above. meetsThreshold (the dollar minimum from Settings) gates SMS,
  // email, AND push alike -- someone who set a $100 minimum shouldn't get
  // buzzed on their phone for a $12 deposit any more than they'd get
  // texted for one. Computed once, outside either try/catch below, so a
  // Twilio/APNs/network blip in one channel can never affect the other,
  // and neither can ever fail the split itself.
  let notifyProfile = null;
  let meetsThreshold = trigger === "auto_deposit";
  if (meetsThreshold) try {
    const { data } = await admin
      ...
      .single();
    notifyProfile = data;
    // A never-configured (null) threshold means every qualifying deposit
    // alerts once enabled, same as a literal $0 threshold. Both text and
    // email already shared this one column before push did too -- one
    // dollar figure now gates every channel that's currently active,
    // rather than tracking one per channel.
    ...
    console.error("[runSplit] failed to load notification profile (non-fatal)", err?.message || err);
  }
  ...
  // Push notification -- same dollar threshold as SMS/email above, and
  // fires on every deposit regardless of hasPending too (see comment
  // above). No separate on/off toggle for push itself since registering a
  // device token in the app (lib/native.js's registerForPushNotifications)
  // is itself the opt-in. Sends one push per registered device -- someone
  // can be signed into the app on more than one phone. simple_push_tokens
  // only has rows once both the Aug 2026 migration has been run (see
  ...
  if (meetsThreshold) {
    try {
      ...
      // Expected (and harmless) until the simple_push_tokens migration
      // has been run -- Postgres "relation does not exist" (42P01), same
