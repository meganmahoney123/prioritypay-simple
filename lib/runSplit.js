...
export async function runSplit({
  ...
  // the user (hasPending), so someone doesn't get notified for a deposit
  // that needed no manual action (e.g. nothing connected yet, or it all
  // stayed in the account it landed in). meetsThreshold (the dollar minimum
  // from Settings) gates SMS, email, AND push alike -- someone who set a
  // $100 minimum shouldn't get buzzed on their phone for a $12 deposit any
  // more than they'd get texted for one. Computed once, outside either
  // try/catch below, so a Twilio/APNs/network blip in one channel can never
  // affect the other, and neither can ever fail the split itself.
  if (hasPending) {
    ...
    // Push notification -- same trigger (hasPending) and now the same
    // dollar threshold as SMS/email above. No separate on/off toggle for
    // push itself since registering a device token in the app (lib/
    // native.js's registerForPushNotifications) is itself the opt-in.
    // Sends one push per registered device -- someone can be signed into
    // the app on more than one phone. simple_push_tokens only has rows
    // once both the Aug 2026 migration has been run (see
