...
export async function runSplit({
  ...
  // Minimum deposit-split threshold (see PHASE I, supabase/schema.sql) --
  ...
  if (trigger === "auto_deposit") {
    const { data: thresholdProfile } = await admin
      ...
      .eq("id", userId)
      .single();
    const threshold = Number(thresholdProfile?.min_deposit_threshold ?? 100);
    if (total < threshold) {
  ...
  if (hasPending) {
    try {
      ...
      // defaults to true -- see PHASE M, supabase/schema.sql), since this
      // is the notification that actually gets someone back into the app
      // to act on their split checklist. Still a real, respected toggle
      // though -- Settings lets someone turn it off. A never-configured
      // (null) threshold means every qualifying deposit texts once enabled,
      // same as a literal $0 threshold.
