...
export const TOOL_SCHEMAS = [
  ...
  },
  {
    ...
    description:
      "How much more the user can contribute this year to a SEP IRA and to a Solo 401k, computed from their real trailing income (or a supplied override), entity type, and age bracket, compared against what they've actually sent through PriorityPay so far this year. Flags if they're already over the limit for either plan. This is the same verified math behind PriorityPay's public Solo 401k vs SEP IRA calculator.",
    input_schema: {
      type: "object",
      properties: {
        netIncomeOverride: {
  ...
  },
  {
    ...
    description:
      "Runs the same sole-proprietor vs. LLC vs. S-corp comparison as PriorityPay's public entity-scenario calculator, using the user's real trailing income by default (or supplied figures). Shows the actual dollar difference an S-corp election would make for THIS person, including the true cost of payroll/extra tax prep and the QBI deduction tradeoff -- not a generic rule of thumb.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Compares what the user is actually setting aside for taxes (their Tax Reserve split-rule percentage applied to real trailing income) against a rough estimate of what they'll likely owe, to flag if they look under- or over-reserved. A sanity check, not a filing-accurate number.",
    input_schema: {
      type: "object",
      properties: {
        filingStatus: { type: "string", enum: ["single", "mfj"], description: "Ask the user if unknown. Defaults to single." },
